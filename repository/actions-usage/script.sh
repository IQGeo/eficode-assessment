...
while IFS= read -r REPO; do
  ...
  # Wrap per-repo processing so a single failure doesn't abort the entire script
  {
    # Workflow count
    WORKFLOW_COUNT=$(gh api "/repos/${REPO}/actions/workflows" --jq '.total_count' 2>/dev/null) || WORKFLOW_COUNT="0"
    [[ "$WORKFLOW_COUNT" =~ ^[0-9]+$ ]] || WORKFLOW_COUNT="0"

    # Workflow runs (last 90 days)
    RUNS_DATA=$(gh api --paginate "/repos/${REPO}/actions/runs?created=>=${SINCE}" --jq '.workflow_runs[] | {status: .status, conclusion: .conclusion, started: .run_started_at, updated: .updated_at}' 2>/dev/null) || RUNS_DATA=""
    ...
    # Artifacts size
    ARTIFACTS_SIZE_BYTES=$(gh api --paginate "/repos/${REPO}/actions/artifacts" --jq '[.artifacts[].size_in_bytes] | add // 0' 2>/dev/null) || ARTIFACTS_SIZE_BYTES="0"
    [[ "$ARTIFACTS_SIZE_BYTES" =~ ^[0-9]+$ ]] || ARTIFACTS_SIZE_BYTES="0"
    ARTIFACTS_SIZE_MB=$(echo "scale=2; ${ARTIFACTS_SIZE_BYTES} / 1048576" | bc)

    # Cache usage
    CACHE_SIZE_BYTES=$(gh api "/repos/${REPO}/actions/cache/usage" --jq '.active_caches_size_in_bytes // 0' 2>/dev/null) || CACHE_SIZE_BYTES="0"
    ...
    if [[ "$WORKFLOW_COUNT" =~ ^[0-9]+$ ]] && [ "$WORKFLOW_COUNT" -gt 0 ]; then
      WORKFLOW_IDS=$(gh api "/repos/${REPO}/actions/workflows" --jq '.workflows[].id' 2>/dev/null) || WORKFLOW_IDS=""
      RUNNERS=""
